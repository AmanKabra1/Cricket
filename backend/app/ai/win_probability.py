"""Win-probability + projected-score predictor (heuristic by default).

If a trained artifact exists AND settings.USE_TRAINED_MODEL is true, it's used;
otherwise a transparent cricket heuristic runs (no ML deps needed).
"""
...
from app.ai.features import MatchFeatures, extract_features
from app.ai.schemas import LiveScoreState, WinProbability
...
def _load_model():
    ...
    if _model_loaded:
        return _model
    _model_loaded = True
    if not settings.USE_TRAINED_MODEL:  # heuristic by default
        return None
...
def _insight(f: MatchFeatures, batting_prob: float, projected: int | None) -> str:
    ...
    try:
        from app.ai.llm import complete as _complete  # noqa: PLC0415
